...
class Todo < Ferro::Component::Article
...
class TodoStatus < Ferro::Element::Block
  ...
  def cascade
    ...
    add_child :clear, TodoStatusClear, content: '[clear]', href: nil
  end
...
class TodoList < Ferro::Form::Base
  ...
  def cascade
    ...
    # Ferro elements keep track of their children via
...
class TodoItem < Ferro::Element::Block
  ...
  def toggle_content(completed)
    content.update_state(:active, completed)
    component.update_status
  end
...
class TodoLabel < Ferro::Form::Label
  ...
  def after_create
    ...
    # If it has a true state Ferro will add CSS classes to the
    # related DOM element. Which classes are added is defined
    # by the Ferro::Compositor
    add_state :active
  end
end